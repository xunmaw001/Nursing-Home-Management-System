<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/18da3753d22a4199b5abc44dee037d48.png) fixed"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 0px 0","margin":"0 0 0px","borderColor":"#ececde","borderStyle":"solid","background":"#fff","borderWidth":"1px 1px 0 "}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0 0 20px","margin":"0 auto","borderColor":"#ececde","borderRadius":"0px","textAlign":"center","background":"none","borderWidth":"0 0 1px","display":"block","borderStyle":"solid"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="医护等级" prop="yihudengji">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"50px","fontSize":"16px","fontWeight":"500","height":"50px"}' class="item-label">医护等级</label>
						<el-select  @change="yihudengjiChange" clearable v-model="searchForm.yihudengji" placeholder="请选择医护等级">
							<el-option v-for="(item,index) in yihudengjiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid #85bfeb","cursor":"pointer","padding":"0 10px 0 26px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"20px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat 5px center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('yihurenyuan','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"1px solid #85bfeb","cursor":"pointer","padding":"0 10px 0 26px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"20px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat 5px center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if="isAuth('yihurenyuan','员工档案')" type="success" size="mini" @click="yuangongdanganCrossAddOrUpdateHandler(scope.row,'cross','','','')">员工档案</el-button>
							<el-button :style='{"border":"1px solid #a4dbdd","cursor":"pointer","padding":"0 10px 0 26px","margin":"0 5px 6px","outline":"none","color":"#15a4aa","borderRadius":"20px","background":"url(http://codegen.caihongy.cn/20230329/99f867042acb4c4aae637c8bfcf6714a.png) no-repeat 3px center","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('yihurenyuan','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"1px solid #e68080","cursor":"pointer","padding":"0 10px 0 26px","margin":"0 5px 6px","outline":"none","color":"#c00","borderRadius":"20px","background":"url(http://codegen.caihongy.cn/20230329/c5ea125f385740a3a3b9aed07f5dfe2c.png) no-repeat 5px center / 20px","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('yihurenyuan','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<yuangongdangan-cross-add-or-update v-if="yuangongdanganCrossAddOrUpdateFlag" :parent="this" ref="yuangongdanganCrossaddOrUpdate"></yuangongdangan-cross-add-or-update>
...
import yuangongdanganCrossAddOrUpdate from "../yuangongdangan/add-or-update";
export default {
  data() {
    return {
      yihudengjiOptions: [],
      ...
      yuangongdanganCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    yuangongdanganCrossAddOrUpdate,
  },
  methods: {
    ...
    yuangongdanganCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yuangongdanganCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yihurenyuan');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.yuangongdanganCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.yuangongdanganCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.yihudengji!='' && this.searchForm.yihudengji!=undefined){
            params['yihudengji'] = this.searchForm.yihudengji
          }
      this.$http({
