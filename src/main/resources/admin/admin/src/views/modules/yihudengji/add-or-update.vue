...
export default {
	data() {
		...
		return {
			...
			rules: {
				yihudengji: [
					{ required: true, message: '医护等级不能为空', trigger: 'blur' },
				],
			}
		};
	},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yihudengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yihudengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
