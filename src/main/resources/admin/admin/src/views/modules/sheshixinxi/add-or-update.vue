<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/18da3753d22a4199b5abc44dee037d48.png) fixed"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'" label="设施编号" prop="sheshibianhao">
					<el-input v-model="ruleForm.sheshibianhao" placeholder="设施编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.sheshibianhao" label="设施编号" prop="sheshibianhao">
					<el-input v-model="ruleForm.sheshibianhao" placeholder="设施编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="设施名称" prop="sheshimingcheng">
					<el-input v-model="ruleForm.sheshimingcheng" placeholder="设施名称" clearable  :readonly="ro.sheshimingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="设施名称" prop="sheshimingcheng">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="使用年限" prop="shiyongnianxian">
					<el-input v-model="ruleForm.shiyongnianxian" placeholder="使用年限" clearable  :readonly="ro.shiyongnianxian"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="使用年限" prop="shiyongnianxian">
					<el-input v-model="ruleForm.shiyongnianxian" placeholder="使用年限" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='sheshibianhao'){
							this.ruleForm.sheshibianhao = obj[o];
							this.ro.sheshibianhao = true;
							continue;
						}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../springboot35806/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `sheshixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.sheshixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
