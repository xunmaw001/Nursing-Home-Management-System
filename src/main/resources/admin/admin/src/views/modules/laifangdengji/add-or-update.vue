<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/18da3753d22a4199b5abc44dee037d48.png) fixed"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="来访分类" prop="laifangfenlei">
					<el-select :disabled="ro.laifangfenlei" v-model="ruleForm.laifangfenlei" placeholder="请选择来访分类" >
						<el-option
							v-for="(item,index) in laifangfenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="来访分类" prop="laifangfenlei">
					<el-input v-model="ruleForm.laifangfenlei"
						placeholder="来访分类" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="来访时间" prop="laifangshijian">
					<el-date-picker
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.laifangshijian" label="来访时间" prop="laifangshijian">
					<el-input v-model="ruleForm.laifangshijian" placeholder="来访时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="来访过程" prop="laifangguocheng">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.laifangguocheng" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.laifangguocheng" label="来访过程" prop="laifangguocheng">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.laifangguocheng"></span>
                </el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.laifangshijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='laifangfenlei'){
							this.ruleForm.laifangfenlei = obj[o];
							this.ro.laifangfenlei = true;
							continue;
						}
						if(o=='laifangshijian'){
						...
						if(o=='laifangguocheng'){
							this.ruleForm.laifangguocheng = obj[o];
							this.ro.laifangguocheng = true;
							continue;
						}
				}
            ...
            this.laifangfenleiOptions = "志愿者,社会人员".split(',')
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.laifangguocheng = this.ruleForm.laifangguocheng.replace(reg,'../../../springboot35806/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
