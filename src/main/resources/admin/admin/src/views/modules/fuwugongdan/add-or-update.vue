<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/18da3753d22a4199b5abc44dee037d48.png) fixed"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="服务项目" prop="fuwuxiangmu">
					<el-input v-model="ruleForm.fuwuxiangmu" placeholder="服务项目" clearable  :readonly="ro.fuwuxiangmu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="服务项目" prop="fuwuxiangmu">
					<el-input v-model="ruleForm.fuwuxiangmu" placeholder="服务项目" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="医护工号" prop="yihugonghao">
					<el-select :disabled="ro.yihugonghao" @change="yihugonghaoChange" v-model="ruleForm.yihugonghao" placeholder="请选择医护工号">
						<el-option
							v-for="(item,index) in yihugonghaoOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.yihugonghao" label="医护工号" prop="yihugonghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="时间段" prop="shijianduan">
					<el-input v-model="ruleForm.shijianduan" placeholder="时间段" clearable  :readonly="ro.shijianduan"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="时间段" prop="shijianduan">
					<el-input v-model="ruleForm.shijianduan" placeholder="时间段" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="服务要求" prop="fuwuyaoqiu">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.fuwuyaoqiu" label="服务要求" prop="fuwuyaoqiu">
					<span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.fuwuyaoqiu}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					var json = data.data;
					if(((json.laorenbianhao!=''&&json.laorenbianhao) || json.laorenbianhao==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.laorenbianhao = json.laorenbianhao
						this.ro.laorenbianhao = true;
					}
            ...
            this.$http({
				url: `option/yihurenyuan/yihugonghao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.yihugonghaoOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
			...
			yihugonghaoChange () {
				this.$http({
					url: `follow/yihurenyuan/yihugonghao?columnValue=`+ this.ruleForm.yihugonghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.yihuxingming){
							this.ruleForm.yihuxingming = data.data.yihuxingming
						}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fuwugongdan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fuwugongdanCrossAddOrUpdateFlag = false;
						 this.parent.search();
